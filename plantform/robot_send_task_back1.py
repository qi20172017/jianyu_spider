...
class Translate:
    ...
    def send(self, params, trace_sn, job_id, data_type):
...
def main():
    ...
    while True:
        res_keys = rds_206_11.keys('robot:res*')
        task_keys = rds_206_11.keys('robot:task*')

        if not task_keys and res_keys:
            pass
        else:
            time.sleep(60)
            print('等待60秒~')
        ...
        for res_key in res_keys:
            print(res_key.decode())
            data_type, trance_sn, job_id, task_id, company_name = res_key.decode().split('_')
            data_type = DATA_TYPE_DICT[data_type.split(':')[-1]]
            data = rds_206_11.smembers(res_key.decode())


            res = [json.loads(item.decode()) for item in data]

            data = {
                'company_name': company_name,   # 对于不是company_name的，这里要考虑修改下
                'data': res
            }

            if not trance_sn_list or trance_sn in trance_sn_list:
                # result_data[company_name]: res
                result_data.append(data)
                job_id_list.append(job_id)
                trance_sn_list.append(trance_sn)
                delete_key.append(res_key.decode())

        print(result_data)

        kfk.send(result_data, trance_sn_list[0], job_id_list[0], data_type)
        # print('delete_key: ', delete_key)
        for key in delete_key:
            rds_206_11.delete(key)
